...
| 공격 | 막아 둔 것 | 남는 것 |
|---|---|---|
| 압축 폭탄 (작은 파일 → 거대한 압축 해제) | 라이브러리 기본값은 셀 3,200만 · 채널 4,096 · 샘플 1,048,576 · 문자열당 64 KiB · 파일 512 MiB 이고, 넘으면 `LC_ERR_TOO_LARGE` 로 **자르지 않고 거절**합니다. | **`logscope.exe` 는 이 상한을 모두 끕니다** (아래 3.7 참고). 문자열 길이 상한만 남습니다. |
...
### 3.7 `logscope.exe` 는 크기 상한을 쓰지 않습니다

사용자가 **자기 로그**를 여는 도구이므로, 크기를 이유로 파일을 거절하지 않습니다.
`LC_UNLIMITED` 로 셀·채널·샘플·파일 크기 상한을 모두 끕니다.

무엇이 남고 무엇이 사라지는지 분명히 해 둡니다.

**그대로 남는 방어**
- XmlLite 는 여전히 DTD 를 지원하지 않습니다 (XXE·엔티티 폭탄 불가)
- 패키지 밖을 가리키는 관계는 여전히 따라가지 않습니다
- 셀 참조·공유 문자열·스타일 인덱스의 **범위 검사는 그대로**입니다
- 문자열 하나당 64 KiB 상한은 그대로입니다
- 할당 실패는 `LC_ERR_MEMORY` 로 잡아 돌려줍니다 (크래시가 아니라 오류)

**사라지는 방어**
- 압축 폭탄이 **메모리를 고갈시키는 것**을 막지 못합니다. 작은 `.xlsx` 가 수십 GB로
  펼쳐지면 프로그램이 메모리를 다 쓰고 `LC_ERR_MEMORY` 로 실패하거나, 그 전에
  시스템이 느려질 수 있습니다.

**그래서 이런 뜻입니다.** 남이 보낸 출처가 불분명한 `.xlsx` 를 이 프로그램으로 여는
것은 이제 "거절당한다" 가 아니라 "메모리를 다 쓸 수도 있다" 가 됩니다. 내 장비에서
뽑은 내 로그를 여는 용도라면 문제되지 않지만, 외부에서 받은 파일을 다루는
용도로 바꾼다면 `LcOpenOptions` 의 상한을 다시 켜세요. 라이브러리(`logcore.dll`)의
기본값은 그대로 상한이 걸려 있으므로, 옵션을 건드리지 않는 다른 호출자는 영향을
받지 않습니다.

메모리는 대략 **파일 크기의 3~5배**를 씁니다 (셀마다 종류·숫자·문자열을 들고 있기
때문). 격자에서 채널로 옮긴 행은 바로 비우고 CSV 원본 바이트도 해제하므로 최고점은
그보다 낮지만, 수백 MB 파일이면 GB 단위를 쓴다고 보시면 됩니다.
